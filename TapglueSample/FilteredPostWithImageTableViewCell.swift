//
//  FilteredPostWithImageTableViewCell.swift
//  TapglueSample
//
...
protocol FilteredPostWithImageCellDataUpdater {
    func updateFilteredPostWithImageTableViewData()
    func showFilteredPostWithImageShareOptions(post: Post)
}

class FilteredPostWithImageTableViewCell: UITableViewCell {

    let appDel = UIApplication.sharedApplication().delegate! as! AppDelegate
    
    var delegate: FilteredPostWithImageCellDataUpdater?
    ...
    @IBAction func likeButtonPressed(sender: UIButton) {
        if likeButton.selected == true {
            ...
            appDel.rxTapglue.deleteLike(forPostId: cellPost.id!).subscribe({ (event) in
                ...
                case .Completed:
                    print("Completed")
                    self.delegate?.updateFilteredPostWithImageTableViewData()
    ...
    @IBAction func shareButtonPressed(sender: UIButton) {
        delegate?.showFilteredPostWithImageShareOptions(cellPost)
    }
    ...
    func configureCellWithPost(post: Post!){
        ...
        // PostImageURL
        for att in cellPost.attachments! {
            if att.name == "image" {
